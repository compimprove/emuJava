...
//              ProcessProgress.getProcessProgress().statusLabel.setText( f.getName() );
  ...
  public void displayMutants() {
    ...
    for (int n = 0; n < EMConstants.ALL_TARGETS.size(); n++) {
      Target target = (Target) EMConstants.ALL_TARGETS.get(n);
  ...
  public void updateStatisticsAndResults() {
    ...
    int suspicious = 0;
    int equivalent = 0;
    for (int t = 0; t < EMConstants.ALL_TARGETS.size(); t++) {
    ...
    int nonEquivalent = EMConstants.ALL_TARGETS.size() + EMConstants.ACHIEVED_TARGETS.size();
    int mutScore = 0;
    if (nonEquivalent > 0) {
    ...
    if (GAManager.RUNNING_THREADS == 0) {
      try {
        ...
        operatorsList = new StringBuilder((String) EMConstants.MUTATION_OPERATORS.get(0));

        for (int o = 1; o < EMConstants.MUTATION_OPERATORS.size(); o++) {
          operatorsList.append(", ").append((String) EMConstants.MUTATION_OPERATORS.get(o));
        }
        raf.writeBytes(operatorsList.toString());
        raf.writeBytes("\n");
