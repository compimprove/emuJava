...
public class ClassHeader {
    ...
    public ArrayList<Token> extractImports(ArrayList<Token> tokenList) {
    ...
    public ArrayList<Token> extractHeader(ArrayList<Token> tokenList) {
        ArrayList<Token> headerList = new ArrayList<Token>();
        for (; index < tokenList.size(); index++) {
            Token token = tokenList.get(index);
    ...
    public String getClassParent(ArrayList headerList) {
        ...
        for (int h = 0; h < headerList.size(); h++) {
            Token token = (Token) headerList.get(h);
            if (token.getToken().equals("extend")) {
